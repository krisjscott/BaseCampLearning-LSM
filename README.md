...
- **Google OAuth local troubleshooting is intentionally documented here.** Run the backend with the `dev` profile on `8081`, run the learner frontend on `3100`, set `FRONTEND_URL=http://localhost:3100`, and set `NEXT_PUBLIC_API_BASE_URL=http://localhost:8081`. Google Cloud must contain `http://localhost:8081/login/oauth2/code/google` as an authorized redirect URI. A healthy `/actuator/health` endpoint only proves the API and database are up; it does not prove that the OAuth callback exchange is working.
- **OAuth callback failures can be caused by browser CORS preflight.** The exchange is a browser `POST` from the frontend to `/api/v1/auth/oauth/exchange`, so the backend explicitly allows the frontend origin, `OPTIONS`, and the required request headers. The canonical backend variable is `CORS_ALLOWED_ORIGINS`; `FRONTEND_URL` is also added automatically to the allowlist. If DevTools shows `OPTIONS` but no `POST`, inspect the preflight status and response headers, then restart the backend after changing configuration.
- **The OAuth exchange must remain single-use.** React development Strict Mode can run callback effects more than once; the callback page guards the exchange so the same code is not submitted twice. Do not refresh `/oauth/callback` or reuse an old callback URL—start a fresh Google sign-in attempt instead.
- **The API error handler distinguishes routing errors from application errors.** Requests such as `GET /api/v1/auth/oauth/exchange` are invalid because the endpoint accepts `POST` only and now return `405`; requests for the backend root or other missing resources return `404` instead of being mislabeled as `500 Internal server error`.
